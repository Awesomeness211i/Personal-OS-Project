...
pub enum DevicePathProtocol {
	Hardware(hardware::Type) = 0x01,
	Acpi(acpi::Type) = 0x02,
	Messaging(messaging::Type) = 0x03,
	Media(media::Type) = 0x04,
...
pub mod media {
	...
	#[derive(Clone, Copy, PartialEq, Eq)]
	pub enum Type {
		HardDrive(Data<HardDrive>) = 0x01,
		CdRom(Data<CdRom>) = 0x02,
		Vendor(Data<Vendor>) = 0x03,
		Filepath(Data<()>) = 0x04,
