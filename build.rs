fn main() -> Result<(), std::env::VarError> {
	let output_path = std::path::PathBuf::from(std::env::var("OUT_DIR")?);
	let uefi_path = output_path.join("uefi.img");
	let kernel_path = std::path::PathBuf::from(std::env::var("CARGO_BIN_FILE_KERNEL")?);
	let bootloader_path = std::path::PathBuf::from(std::env::var("CARGO_BIN_FILE_BOOTLOADER")?);
