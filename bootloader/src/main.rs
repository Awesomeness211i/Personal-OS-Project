...
pub unsafe extern "efiapi" fn main(image_handle: &mut c_void, system_table: SystemTablePointer) -> Status {
	...
	let kernel = {
		...
		let info_address = PhysicalAddress::new(0);
		let status = unsafe { (system_table.boot_services().allocate_pool)(MemoryType::LOADER_DATA, info_size, &mut info_address.to_ptr()) };
		if status != Status::SUCCESS {
			panic!("Failed to allocate page with status: {status}")
		}
		let status = unsafe { ((*kernel_file).get_info)(kernel_file, &FileInfo::GUID, &mut info_size, info_address.to_ptr()) };
		if status != Status::SUCCESS {
			panic!("Failed to get info with: {status}, {info_size}")
		}

		let info = unsafe { info_address.to_ptr::<FileInfoHeader>().read() };
		...
		Elf::new(address.to_ptr(), size).unwrap()
	};
	...
	let base_address: u64 = if kernel_elf_header.executable_type == ExecutableType::DYNAMIC {
	...
	match system_table.exit_boot_services(image_handle, map_key) {
		Ok(system_table) => {
			...
			for i in 0..memory_map_size / descriptor_size {
				let descriptor = unsafe { &mut *(memory_map.add(i * descriptor_size) as *mut MemoryDescriptor) };
				match descriptor.region_type {
					MemoryType::BOOT_SERVICES_CODE | MemoryType::BOOT_SERVICES_DATA | MemoryType::CONVENTIONAL_MEMORY => {
			...
			let mut kernel_address_space = AddressSpace::create(address_space_physical, address_space_page_num as usize, 4);
			...
			for i in 0..memory_map_size / descriptor_size {
				...
				match descriptor.region_type {
					...
					MemoryType::RUNTIME_SERVICES_CODE | MemoryType::RUNTIME_SERVICES_DATA => {
						let address = descriptor.physical_start;
						for i in 0..descriptor.num_pages {
							kernel_address_space.mmap(
								VirtualAddress::new((address.get() + (i << 12)) as u64),
								PhysicalAddress::new(address.get() + (i << 12) as u64),
								EntryFlags::PRESENT,
								EntryFlags::PRESENT,
							);
						}
					...
					MemoryType::PERSISTENT_MEMORY => {},
					KERNEL_PAGE => {
						let kernel_pages = descriptor.physical_start;
							...
							match program_header.p_type {
								ProgramHeaderType::LOAD => {
									...
									for i in 0..segment_pages {
										kernel_address_space.mmap(
											VirtualAddress::new(base_address as u64 + ((used_kernel_pages + i) << 12) as u64),
											PhysicalAddress::new(kernel_pages.get() + ((used_kernel_pages + i) << 12) as u64),
								...
								ProgramHeaderType::DYNAMIC => {
									...
									let mut i = 0;
									loop {
										let dynamic_entry = unsafe { kernel.offset(offset + i * size_of::<Elf64Dynamic>()) };
										...
										for i in 0..rela_size / rela_entry_size {
											...
											match rela_type {
												...
												Elf64RTypeX86_64::R_AMD64_RELATIVE => {
													...
													let off = kernel_pages.get() + offset;
													// TODO: Check that this is correct
													println(format_args!("writing: {relative:X} to: {off:X}"));
													unsafe {
														(kernel_pages.to_ptr::<u8>().add(offset as usize) as *mut u64).write(relative);
													}
												},
									...
									if rel_entry_size > 0 {
										...
										for i in 0..rel_size / rel_entry_size {
											let rel_entry = unsafe { kernel.offset::<Elf64Rel>(rel as usize + i as usize * rel_entry_size as usize) };
					...
					STACK_PAGE => {
						stack = Some(VirtualAddress::new(
							base_address + ((1 + kernel_page_count + kernel_arguments_page_count + stack_page_count) << 12) as u64,
						));
						for i in 0..descriptor.num_pages {
							kernel_address_space.mmap(
								VirtualAddress::new(base_address + ((1 + kernel_page_count as u64 + kernel_arguments_page_count as u64 + i) << 12)),
								PhysicalAddress::new(descriptor.physical_start.get() + ((i as u64) << 12) as u64),
								EntryFlags::PRESENT | EntryFlags::READ_WRITE,
								EntryFlags::PRESENT | EntryFlags::READ_WRITE,
							);
						}
					...
					ARGUMENTS => {
						kernel_arguments = Some(VirtualAddress::new(base_address + (kernel_page_count << 12) as u64));
						for i in 0..descriptor.num_pages {
							kernel_address_space.mmap(
								VirtualAddress::new(base_address + ((kernel_page_count as u64 + i) << 12)),
								PhysicalAddress::new(descriptor.physical_start.get() + ((i as u64) << 12)),
								EntryFlags::PRESENT | EntryFlags::READ_WRITE,
								EntryFlags::PRESENT,
							);
						}
			...
			unsafe {
				...
				core::arch::asm!(
					...
					in("rdx") kernel_address_space.get_ptr(),
		...
		Err(system_table) => {
			...
			println(format_args!(
				"Memory Map Pointer: {memory_map:#X?}, Memory Map Key: {map_key}, Memory Map Size: {memory_map_size}, Memory Map Descriptor Size: {descriptor_size}, Memory Map Descriptor Version: {descriptor_version}"
			));
			Status::ABORTED
		},
	}
