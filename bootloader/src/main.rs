...
const ADDRESS_SPACE: MemoryType = MemoryType::custom_os(0x80000003);
...
pub extern "efiapi" fn main(image_handle: &mut c_void, system_table: SystemTablePointer) -> Status {
	...
	let mut kernel_address_space = {
		let page_number = 13;
		let mut address = PhysicalAddress::new(0);
		let status = unsafe { (system_table.boot_services().allocate_pages)(AllocateType::ANY_PAGES, ADDRESS_SPACE, page_number, &mut address) };
		if status != Status::SUCCESS {
			panic!("Failed to allocate page with status: {status}")
		}
		AddressSpace::create(address, page_number, 4)
	};
	...
	let trampoline = {
		...
		let flags = EntryFlags::PRESENT | EntryFlags::READ_WRITE;
		kernel_address_space.mmap(VirtualAddress::new(address.get()), address, EntryFlags::PRESENT | EntryFlags::READ_WRITE, flags);
		unsafe { address.to_ptr::<Page>().write_bytes(0, number) };
		address
	};

	// Allocate stack pages
	let (stack, stack_page_count) = {
		let number = 5;
		let mut address = PhysicalAddress::new(0);
	...
	for i in (0..stack_page_count).rev() {
		let flags = EntryFlags::PRESENT | EntryFlags::READ_WRITE;
		kernel_address_space.mmap(
			VirtualAddress::new(0u64.wrapping_sub(((i + 1) as u64) << 12)),
			PhysicalAddress::new(stack.get() + ((stack_page_count - i) << 12) as u64),
	...
	let (kernel_pages, kernel_page_count) = {
	...
	for i in 0..kernel_program_header_num.get() {
		...
		match program_header.p_type {
			ProgramHeaderType::LOAD => {
				...
				if used_kernel_pages + segment_pages > kernel_page_count {
				...
				for i in 0..segment_pages {
					kernel_address_space.mmap(
						...
						PhysicalAddress::new(kernel_pages.get() + (used_kernel_pages << 12) as u64 + (i << 12) as u64),
						EntryFlags::PRESENT | EntryFlags::READ_WRITE,
						page_permissions,
					);
				}
				...
				if file_size > 0 {
					...
					set_position(kernel_file, file_offset as u64);
					while bytes_to_write != 0 {
						let status = unsafe { ((*kernel_file).read)(kernel_file, &mut bytes_to_write, kernel_pages.to_ptr::<u8>().add((used_kernel_pages << 12) + offset + pos) as *mut _) };
						if status != Status::SUCCESS {
							panic!("More huh?! {status} {file_offset}");
						}
						pos += bytes_to_write;
				...
				if zero_fill_count > 0 {
					...
					unsafe { kernel_pages.to_ptr::<u8>().add((used_kernel_pages << 12) + offset + file_size).write_bytes(0, zero_fill_count) };
				}
					...
					for i in 0..rela_size / rela_entry_size {
						...
						match rela_type {
							...
							Elf64RTypeX86_64::R_AMD64_RELATIVE => {
								...
								unsafe {
									(kernel_pages.to_ptr::<u8>().add(offset as usize) as *mut i64).write(relative);
								}
							},
	...
	match system_table.exit_boot_services(image_handle, map_key) {
		Ok(system_table) => {
			...
			println(format_args!("Entry: {kernel_entry:X?}"));
			...
			unsafe {
				core::arch::asm!(
					...
					size = in(reg) size,
					trampoline = in(reg) trampoline.to_ptr::<u8>(),
					address_space = in(reg) kernel_address_space.get_ptr(),
